...
import com.retrivedmods.wclient.game.utils.constants.Enchantment
import com.retrivedmods.wclient.game.utils.misc.getEnchant
...
class AutoArmorModule : Module("AutoArmor", ModuleCategory.Combat) {

    private var delayMs by intValue("Delay", 50, 1..1000)
    private var preferProtection by boolValue("Prefer Protection", true)
    private var dropReplacedArmor by boolValue("Drop replaced armor", false)
    ...
    override fun beforePacketBound(interceptablePacket: InterceptablePacket) {
        ...
        if (now - lastEquipTime < delayMs || isEquipping) return
    ...
    private suspend fun checkAndEquipBestArmor() {
        ...
        for ((armorSlot, armorType) in armorSlots) {
            val currentArmor = inventory.content.getOrNull(armorSlot) ?: ItemData.AIR
    ...
    private fun findBestArmorInInventory(armorType: String, inventory: PlayerInventory): Int? {
        ...
        for (i in 0 until 36) {
            val item = inventory.content.getOrNull(i) ?: ItemData.AIR
